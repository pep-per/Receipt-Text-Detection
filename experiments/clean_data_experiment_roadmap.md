...
- 상세 기록: [V11B Five-fold OOF](20260714-v11b-oof/README.md)
- Pooled OOF macro H/P/R: `0.946938 / 0.950739 / 0.947650`
- Pooled OOF global H/P/R: `0.947775 / 0.950045 / 0.945515`
- Fold macro/global H 표준편차: `0.009142 / 0.007027`
- Worst-fold macro/global H: `0.932941 / 0.936876`
- 3,676장 exact-one OOF coverage만 통과하고 나머지 네 stability gate는 실패
- 결정: fixed epoch-8 recipe를 stable teacher로 채택하지 않고 V11C를 보류

낮은 Fold 3은 기존 train/val 출처, small-text 여부와 region-count 구간 전반에서 함께 낮았다.
Manifest의 측정 strata는 균형이었고 checkpoint 무결성도 통과했으므로, 현재 constant-LR
recipe가 학습 subset 또는 측정하지 못한 receipt cluster에 민감하다는 신호다.

다만 원래 V2B epoch 8은 `1,845` optimizer update를 사용했지만 fold model의 epoch 8은 train
set이 작아 `1,656` update만 사용했다. 이 `10.2%` budget 차이를 제거하기 전에는 fold 분산을
architecture 자체의 한계로 단정하지 않는다. 다음 실행은 `V11B-R1`이며, 모든 fold의 epoch-8
checkpoint를 똑같이 한 epoch 재개해 약 `1,840` update의 fixed epoch-9 state를 평가한다. Split,
seed, threshold와 사전 gate는 유지하고 fold별로 epoch를 고르지 않는다.
...
V11B가 stability gate를 실패했으므로 현재 V11C는 보류 상태다. V11B-R1이 같은 gate를
통과하거나 후속 seed audit이 분산 원인을 설명하기 전에는 단순 평균이 불안정성을 해결했다고
간주하지 않는다.
...
| `V11B-R1` | Matched-update five-fold OOF | V11B 뒤, V11C 전 | V11B가 V2B보다 10.2% 적은 update로 stability gate를 실패했으므로 실행. 모든 fold를 epoch 9까지 동일하게 재개하고 기존 split/gate 유지 |
...
통과해 고정했다. V11B fixed epoch-8 OOF는 fold stability gate를 실패했고 원래 V2B보다
update가 10.2% 적었다. 따라서 다음 실험은 같은 fold를 한 epoch씩 재개하는 `V11B-R1`
matched-update OOF이며, V11C와 test artifact 생성은 그 결과까지 보류한다.

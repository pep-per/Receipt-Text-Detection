...
| `baseline_code/outputs/` | 약 3.5GB | 체크포인트, Hydra 설정, 예측 JSON | 필수 체크포인트 포함 |
| `baseline_code/wandb/` | 약 16MB | 로컬 W&B run metadata | 온라인 sync 후에는 선택 사항 |
...
- V10 SSL 재현: `v10_ssl_moco/encoder_state_dict.pt`, V10 epoch-8 detector checkpoint,
  별도 보관한 공식 데이터와 `data/pseudo_label/` 후보 데이터
- V11 K-fold 기본 초기화: V10이 동률로 폐기되어 V2B와 같은 ImageNet ResNet18 사용

...
class CLEvalMetric(Metric):
    ...
    def accumulate_metric(self, metric):
        """Add another metric's raw CLEval counts without repeating geometry evaluation."""
        for state_name in self.ACCUMULATED_STATE_NAMES:
            target = getattr(self, state_name)
            source = getattr(metric, state_name).detach().to(target.device)
            target.add_(source)
        self._update_count += metric._update_count
        self._computed = None
